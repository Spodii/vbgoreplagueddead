...
Public Const EffectNum_EquationTemplate As Byte = 8 'Template for creating particle effects through equations - a page with some equations can be found here: http://www.vbgore.com/Particle_effect_equations
Public Const EffectNum_Waterfall As Byte = 9        'Waterfall effect
Public Const EffectNum_Summon As Byte = 10          'Summon effect
Public Const EffectNum_BurningHands As Byte = 11    'Burning hands spell
Public Const EffectNum_ChangeClass As Byte = 12     'When the user changes their class
...
Function Effect_ChangeClass_Begin(ByVal X As Single, ByVal Y As Single, ByVal Gfx As Integer, ByVal Particles As Integer, Optional ByVal Progression As Single = 1) As Integer
Dim EffectIndex As Integer
Dim LoopC As Long
...
Dim i As Long
    ...
    If Effect(EffectIndex).EffectNum = EffectNum_BloodSpray Or Effect(EffectIndex).EffectNum = EffectNum_BloodSplatter Then
        ...
        D3DDevice.SetTexture 0, ParticleTexture(17)
...
Sub Effect_Update(ByVal LoopC As Integer)
    ...
    If Effect(LoopC).EffectNum = EffectNum_BurningHands Then Effect_BurningHands_Update LoopC
...
Function Effect_BurningHands_Begin(ByVal X As Single, ByVal Y As Single, ByVal Gfx As Integer, ByVal Particles As Integer, Optional ByVal Progression As Single = 1) As Integer
Dim EffectIndex As Integer
Dim LoopC As Long
    ...
    'Return the index of the used slot
    Effect_BurningHands_Begin = EffectIndex

    'Set The Effect's Variables
    Effect(EffectIndex).EffectNum = EffectNum_BurningHands      'Set the effect number
    Effect(EffectIndex).ParticleCount = Particles       'Set the number of particles
    Effect(EffectIndex).Used = True     'Enabled the effect
    Effect(EffectIndex).X = X           'Set the effect's X coordinate
    Effect(EffectIndex).Y = Y           'Set the effect's Y coordinate
    ...
    For LoopC = 0 To Effect(EffectIndex).ParticleCount
        ...
        Effect(EffectIndex).Particles(LoopC).Used = True
        Effect(EffectIndex).PartVertex(LoopC).Rhw = 1
        Effect_BurningHands_Reset EffectIndex, LoopC
    Next LoopC
...
Private Sub Effect_BurningHands_Reset(ByVal EffectIndex As Integer, ByVal Index As Long)

    'Reset the particle
    Effect(EffectIndex).Particles(Index).ResetIt Effect(EffectIndex).X - 5 + Rnd * 10, Effect(EffectIndex).Y - 5 + Rnd * 10, -Sin((180 + (Rnd * 90) - 45) * 0.0174533) * 12 + (Rnd * 3), Cos((180 + (Rnd * 90) - 45) * 0.0174533) * 12 + (Rnd * 3), 0, 0
    Effect(EffectIndex).Particles(Index).ResetColor 1, 0, 0, 0.8 + (Rnd * 0.2), 0.04 + (Rnd * 0.5)
    
End Sub

Private Sub Effect_BurningHands_Update(ByVal EffectIndex As Integer)
Dim ElapsedTime As Single
Dim LoopC As Long
Dim i As Integer
