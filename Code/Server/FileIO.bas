...
Private TempObjs() As udtObjData
...
Dim i As Long
    ...
    Get #FileNumMap, , MapInfo(MapNum).PVP
    Get #FileNumMap, , MapInfo(MapNum).Job
    ...
    For Y = 1 To MapInfo(MapNum).Height
        For X = 1 To MapInfo(MapNum).Width
            ...
            Get #FileNumMap, , MapInfo(MapNum).Data(X, Y).LightIntensity
...
Dim l As Long
    ...
    Do While DB_RS.EOF = False  'Loop until we reach the end of the recordset
        ...
        With NPCList(1)
            ...
            .BaseStat(SID.AttackDelay) = Val(DB_RS!stat_attackdelay)
            .BaseStat(SID.Str) = Val(DB_RS!stat_str)
            .BaseStat(SID.Agi) = Val(DB_RS!stat_agi)
            .BaseStat(SID.Speed) = Val(DB_RS!stat_speed)
            .BaseStat(SID.Inte) = Val(DB_RS!stat_int)
            .BaseStat(SID.DEF) = Val(DB_RS!stat_def)
            .BaseStat(SID.Accuracy) = Val(DB_RS!stat_accuracy)
            .BaseStat(SID.Armor) = Val(DB_RS!stat_armor)
            .BaseStat(SID.Brave) = Val(DB_RS!stat_brave)
            .BaseStat(SID.Dex) = Val(DB_RS!stat_dex)
            .BaseStat(SID.Evade) = Val(DB_RS!stat_evade)
            .BaseStat(SID.Immunity) = Val(DB_RS!stat_immunity)
            .BaseStat(SID.Perception) = Val(DB_RS!stat_perception)
            .BaseStat(SID.Recov) = Val(DB_RS!stat_recov)
            .BaseStat(SID.Regen) = Val(DB_RS!stat_regen)
            .BaseStat(SID.Tactics) = Val(DB_RS!stat_tactics)
            .BaseStat(SID.WeaponSkill) = Val(DB_RS!stat_weaponskill)
            .BaseStat(SID.MaxHP) = Val(DB_RS!stat_hp)
            .BaseStat(SID.MaxEP) = Val(DB_RS!stat_ep)
            
            .NPCNumber = DB_RS!id
            .Flags.NPCActive = 1
            .BaseStat(SID.EXP) = 1  'EXP multiplier
            ShopStr = Trim$(DB_RS!objs_shop)
            DropStr = Trim$(DB_RS!drops)
            
            'Calculate the NPC's level / exp
            EXP = 0
            Level = 1
            For Stat = 1 To NumStats
                Select Case Stat
                    Case SID.ELU
                    Case SID.ELV
                    Case SID.EXP
                    ...
                    'HP and EP
                    Case SID.MaxHP, SID.MaxEP
                        If .BaseStat(Stat) > 50 Then
                            For l = 1 To .BaseStat(Stat) - 50
                                EXP = EXP + (StatCost(l) \ 20)
                                Do While LevelCost(Level) <= EXP
                                    EXP = EXP - LevelCost(Level)
                                    Level = Level + 1
                                Loop
                            Next l
                        End If
                    ...
                    'Damage
                    Case SID.MinHIT, SID.MaxHIT
                        For l = 1 To .BaseStat(Stat)
                            EXP = EXP + (StatCost(l) \ 2)
                            Do While LevelCost(Level) <= EXP
                                EXP = EXP - LevelCost(Level)
                                Level = Level + 1
                            Loop
                ...
                End Select
            Next Stat
            .BaseStat(SID.ELV) = Level
            .BaseStat(SID.EXP) = ((Level + (EXP / LevelCost(Level))) * 11) ^ 0.9
            ...
            'Replenish the HP/EP
            .BaseStat(SID.MinHP) = .ModStat(SID.MaxHP)
            .BaseStat(SID.MinEP) = .ModStat(SID.MaxEP)
...
Dim FileNum As Byte
    ...
    'Resize the array to fit all the objects
    ReDim TempObjs(1 To NumObjDatas)
    ...
    Do While DB_RS.EOF = False  'Loop until we reach the end of the recordset
        With TempObjs(Val(DB_RS!id))
            ...
            .AddStat(SID.MaxEP) = Val(DB_RS!stat_ep)
            .AddStat(SID.Accuracy) = Val(DB_RS!stat_accuracy)
            .AddStat(SID.Armor) = Val(DB_RS!stat_armor)
            .AddStat(SID.Brave) = Val(DB_RS!stat_brave)
            .AddStat(SID.Dex) = Val(DB_RS!stat_dex)
            .AddStat(SID.Evade) = Val(DB_RS!stat_evade)
            .AddStat(SID.Immunity) = Val(DB_RS!stat_immunity)
            .AddStat(SID.Perception) = Val(DB_RS!stat_perception)
            .AddStat(SID.Recov) = Val(DB_RS!stat_recov)
            .AddStat(SID.Regen) = Val(DB_RS!stat_regen)
            .AddStat(SID.Tactics) = Val(DB_RS!stat_tactics)
            .AddStat(SID.WeaponSkill) = Val(DB_RS!stat_weaponskill)
            .AddStat(SID.AttackDelay) = Val(DB_RS!stat_attackdelay)

            .ReqStr = Val(DB_RS!req_str)
            .ReqAgi = Val(DB_RS!req_agi)
            .ReqInt = Val(DB_RS!req_int)
...
Public Sub Load_User(ByVal UserIndex As Integer, ByVal UserName As String, Optional ByVal GM As Byte = 0, Optional ByVal BankGold As Long = 0, Optional ByVal BankStr As String = vbNullString)
...
Dim i As Long
    ...
    'Set the values from the account table
    UserList(UserIndex).BankGold = BankGold
    ...
    With DB_RS
        ...
        MailStr = !mail
        KSStr = !KnownSkills
        CompQStr = Trim$(!quests_completed)
        CurQStr = Trim$(!quests_current)
        UserList(UserIndex).BytesIn = Val(!bytes_in)
        UserList(UserIndex).BytesOut = Val(!bytes_out)
        ...
        UserList(UserIndex).Stats.BaseStat(SID.AttackDelay) = 1000
        ...
        UserList(UserIndex).Stats.BaseStat(SID.Inte) = Val(!stat_int)
        UserList(UserIndex).Stats.BaseStat(SID.DEF) = 0
        ...
        UserList(UserIndex).Stats.BaseStat(SID.ELU) = LevelCost(UserList(UserIndex).Stats.BaseStat(SID.ELV))
        UserList(UserIndex).Stats.BaseStat(SID.Accuracy) = Val(!stat_accuracy)
        UserList(UserIndex).Stats.BaseStat(SID.Armor) = Val(!stat_armor)
        UserList(UserIndex).Stats.BaseStat(SID.Brave) = Val(!stat_brave)
        UserList(UserIndex).Stats.BaseStat(SID.Dex) = Val(!stat_dex)
        UserList(UserIndex).Stats.BaseStat(SID.Evade) = Val(!stat_evade)
        UserList(UserIndex).Stats.BaseStat(SID.Immunity) = Val(!stat_immunity)
        UserList(UserIndex).Stats.BaseStat(SID.Perception) = Val(!stat_perception)
        UserList(UserIndex).Stats.BaseStat(SID.Recov) = Val(!stat_recov)
        UserList(UserIndex).Stats.BaseStat(SID.Regen) = Val(!stat_regen)
        UserList(UserIndex).Stats.BaseStat(SID.Tactics) = Val(!stat_tactics)
        UserList(UserIndex).Stats.BaseStat(SID.WeaponSkill) = Val(!stat_weaponskill)
        UserList(UserIndex).Stats.BaseStat(SID.Rage) = Val(!stat_rage)
        UserList(UserIndex).Stats.BaseStat(SID.Concussion) = Val(!stat_concussion)
        UserList(UserIndex).Stats.BaseStat(SID.Rend) = Val(!stat_rend)
        UserList(UserIndex).Stats.BaseStat(SID.Bloodlust) = Val(!stat_bloodlust)
        UserList(UserIndex).Stats.BaseStat(SID.Points) = Val(!stat_points)
        UserList(UserIndex).Stats.BaseStat(SID.MinHIT) = 0
        UserList(UserIndex).Stats.BaseStat(SID.MaxHIT) = 0
        UserList(UserIndex).Stats.BaseStat(SID.MaxHP) = Val(!stat_hp_max)
        UserList(UserIndex).Stats.BaseStat(SID.MaxEP) = Val(!stat_ep_max)
        UserList(UserIndex).Stats.ModStat(SID.MaxHP) = UserList(UserIndex).Stats.BaseStat(SID.MaxHP)
        UserList(UserIndex).Stats.ModStat(SID.MaxEP) = UserList(UserIndex).Stats.BaseStat(SID.MaxEP)
        UserList(UserIndex).inChannel(1) = ((!inChannel And 1) > 0)
        UserList(UserIndex).inChannel(2) = ((!inChannel And 2) > 0)
        UserList(UserIndex).inChannel(3) = ((!inChannel And 4) > 0)
        UserList(UserIndex).inChannel(4) = ((!inChannel And 8) > 0)
        For i = 1 To NumChannels
            If UserList(UserIndex).inChannel(i) > 0 Then UserList(UserIndex).inChannel(i) = 1
        Next i
        
        'We have to wait until we know the modified max stats, we can set the minimum, or else the user will never be able to log
        ' in with their stats full if they have +HP or +EP stat modifiers equipped
        tMinHP = Val(!stat_hp_min)
        tMinEP = Val(!stat_ep_min)
    ...
    UserList(UserIndex).Stats.BaseStat(SID.MinEP) = tMinEP
    
    'Set the name
    UserList(UserIndex).Name = UserName
    
    'Set the time the user logged in to now
    UserList(UserIndex).LoginTime = timeGetTime
...
Dim i As Long
    ...
    With UserChar
        ...
        'Channel settings
        inChannel = (.inChannel(1) Or 1) Or (.inChannel(2) Or 2) Or (.inChannel(3) Or 4) Or (.inChannel(4) Or 8)
        ...
        DB_RS!quests_completed = CompQStr
        DB_RS!quests_current = CurQStr
        DB_RS!pos_x = .Pos.X
        DB_RS!pos_y = .Pos.Y
        ...
        DB_RS!stat_int = .Stats.BaseStat(SID.Inte)
        ...
        DB_RS!stat_ep_min = .Stats.BaseStat(SID.MinEP)
        DB_RS!stat_ep_max = .Stats.BaseStat(SID.MaxEP)
        DB_RS!stat_accuracy = .Stats.BaseStat(SID.Accuracy)
        DB_RS!stat_armor = .Stats.BaseStat(SID.Armor)
        DB_RS!stat_brave = .Stats.BaseStat(SID.Brave)
        DB_RS!stat_dex = .Stats.BaseStat(SID.Dex)
        DB_RS!stat_evade = .Stats.BaseStat(SID.Evade)
        DB_RS!stat_immunity = .Stats.BaseStat(SID.Immunity)
        DB_RS!stat_rage = .Stats.BaseStat(SID.Rage)
        DB_RS!stat_rend = .Stats.BaseStat(SID.Rend)
        DB_RS!stat_bloodlust = .Stats.BaseStat(SID.Bloodlust)
        DB_RS!stat_concussion = .Stats.BaseStat(SID.Concussion)
        DB_RS!stat_perception = .Stats.BaseStat(SID.Perception)
        DB_RS!stat_recov = .Stats.BaseStat(SID.Recov)
        DB_RS!stat_regen = .Stats.BaseStat(SID.Regen)
        DB_RS!stat_tactics = .Stats.BaseStat(SID.Tactics)
        DB_RS!stat_weaponskill = .Stats.BaseStat(SID.WeaponSkill)
        DB_RS!time_online = ((timeGetTime - .LoginTime) \ 1000) + CLng(DB_RS!time_online)
        DB_RS!bytes_in = .BytesIn
        DB_RS!bytes_out = .BytesOut
        DB_RS!inChannel = inChannel
    ...
    'Update the account
    DB_RS.Open "SELECT name,gm,bankgold,bank FROM accounts WHERE `name`='" & UserList(UserIndex).Account & "'", DB_Conn, adOpenStatic, adLockOptimistic
    DB_RS!GM = UserList(UserIndex).Flags.GMLevel
    DB_RS!BankGold = UserList(UserIndex).BankGold
    DB_RS!Bank = BankStr
    DB_RS.Update
    DB_RS.Close
...
Public Sub Save_ObjsDat()

'*****************************************************************
'Saves the Objs.dat for the client
'*****************************************************************
Dim Path As String
Dim FileNum As Byte
...
Dim b() As Byte
    ...
    'Check if to use the Objs.dat
    If MakeObjsDat Then
        
        'Delete the old file if it exists
        If Dir$(DataPath & "Objs.dat") <> vbNullString Then Kill DataPath & "Objs.dat"
    
        'Open the file
        FileNum = FreeFile
        Open DataPath & "Objs.dat" For Binary Access Write As #FileNum
        
            'Save the header (number of objects)
            Put #FileNum, , NumObjDatas
        
            'Save the objects
            Put #FileNum, , TempObjs()
            
        Close #FileNum
            
        'Encrypt the file
        Encryption_Twofish_EncryptFile DataPath & "Objs.dat", DataPath & "Objs.dat", "1ZXV014!@#)(9487AZXC<>*xv;asfj!)$@"
